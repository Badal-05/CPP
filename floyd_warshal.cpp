...
int main()
{
  ...
  for (i = 1; i <= n; i++)   // 'i' index generates the 4 iterations needed for individually exploring routes via //generates intermidiate(via) node
  {                          // 'j' index generates the row number of the matrices // generates starting node
    for (j = 1; j <= n; j++) // 'j' index generates the row number of the matrices // generates ending node
    {
